...
intent, decisions, rejected-log, prev-handoff, `rca/rca.md?`, `evidence/evidence.md?`(path, 선택), repro-lock 조각(있으면), **check-failure 조각**(사용자가 검사 실패 후 이 노드로 보낸 경우: 실행한 검사, 종료 코드, 로그 끝부분), gate-policy, next-options.
...
1. 들어온 이유를 확인한다: 첫 실행인가, 검사 실패나 verify 판정으로 돌아왔는가(check-failure 조각, prev-handoff). 실패로 돌아왔다면 실패 로그부터 분석하고, 이전 fix 시도 중 실패한 접근은 `rejected`에 적는다.
...
**검사가 실패하면** 세션이 살아 있는 채로 승인 패널에 실패가 표시된다. 사람이 터미널에서 고쳐 달라고 하면 같은 세션에서 고치고 `_close`를 다시 한다(새 task를 열지 않음, D38). 재현 테스트를 바꿔야 한다고 판단되면 바꾸지 말고 이유를 `decisions`(`requires_human: true`)에 적는다 — 사람이 승인 화면에서 [재고정]을 고를 수 있다.
...
없음. 검사 실패나 verify 판정으로 재진입했을 때는 본문 "다음 task가 알아야 할 것"에 이번 시도에서 바꾼 점을 적는다.

## 하지 말 것

- 테스트를 약하게 고치거나(단언 제거, skip, 기대값을 실제값으로 바꾸기) 재현 테스트·테스트 설정(`check_config_globs`)을 수정해서 통과시키지 않는다. 앱이 `repro:intact`와 `tests:unchanged`로 확인하고, final-verify가 다시 점검한다.
