...
- 검증 명령: node "<RELAY_HOME>/bin/relay-validate.js" handoff "<RELAY_TASK_DIR>/handoff.md"
...
1. **초안 먼저, 질문은 나중.** 컨텍스트와 코드로 추론할 수 있는 것은 먼저 채우고, 사람에게는 "이렇게 이해했는데 맞나요?" 형태로 확인만 받는다.
2. **한 번에 묶어서.** 결정 지점 질문은 한 메시지에 모아서 묻는다. 번호를 붙이고, 각 질문에 **추천안**을 적는다. 사람이 "추천대로"라고 답할 수 있어야 한다.
3. **질문 예산.** 스킬마다 결정 지점을 명세에 고정한다. 목록에 없는 것은 묻지 않고 AI가 정한 뒤 `decisions`에 `by: ai, requires_human: false`로 기록한다.
4. **"알아서 해" 처리.** 사람이 위임하면 추천안을 택하고 `by: ai`로 기록한다. 그 결정이 `requires_human: true` 항목이면 자동 승인이 막히므로(의도적), 승인 화면에서 사람이 한 번 더 본다.
5. **작은 work는 짧게.** intent의 `size: S`면 결정 지점 중 "S에서는 생략" 표시된 것은 묻지 않는다.
6. 에이전트의 세션 중 질문에 앱은 절대 자동으로 답하지 않는다(design.md 1.2).
