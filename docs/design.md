...
- **규모 제안 기준:** 8절 2번에서 정한다.
- **형식 검사:** 앱이 초안을 handoff와 같은 방식으로 검사한다(D38).
- 입력, 완료조건, 산출물 템플릿의 나머지는 다음 라운드에서 정한다.
...
1. **스킬 명세(5.6):** work-start의 나머지(입력, 완료조건, 산출물 템플릿). evidence, root-cause, fix, final-verify 각각의 입력, 결정 지점(사람이 정할 결정 포함), 완료조건, 산출물 템플릿. root-cause는 intent `추가 의견`의 사람 추정을 확인하거나 기각해야 하는지도 정한다
