...
1. **스킬 명세(5.6):** root-cause, fix, final-verify 각각의 입력, 결정 지점(사람이 정할 결정 포함), 완료조건, 산출물 템플릿. root-cause는 intent `추가 의견`의 사람 추정을 확인하거나 기각해야 하는지도 정한다. final-verify는 재현 없이 진행한 Work(D45)에서 첫 완료조건을 어떻게 판정하는지도 정한다
