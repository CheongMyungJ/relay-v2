...
스킬은 task 하나에서 에이전트가 따르는 절차다. 이 절에는 모든 스킬에 공통인 규칙(질문 규칙, 공통 종료 절차, 배포와 입력)을 적는다. 스킬별 명세(입력, 결정 지점, 완료조건, 산출물 템플릿)는 5.6.4부터 적는다. 아직 정하지 않은 스킬은 8절 1번에 남아 있다.
...
- **규모:** 초안에 `size`를 제안하고, 근거는 handoff `decisions`에 적는다(D42). 제안 기준은 8절 2번에서 정한다.
...
- **입력:** `context.md`와 `evidence.md`(경로). S 경로에서는 rca를 건너뛴다(8절 2번).
...
- **입력:** `context.md`, `rca.md`(경로), 필요하면 `evidence.md`(경로). S 경로의 입력은 8절 2번에서 정한다.
...
1. **스킬 명세(5.6):** final-verify의 입력, 결정 지점(사람이 정할 결정 포함), 완료조건, 산출물 템플릿. final-verify는 재현 없이 진행한 Work(D45)에서 첫 완료조건을 어떻게 판정하는지도 정한다
