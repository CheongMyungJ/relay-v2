...
4. **앱:** `context.md`를 조립한다.
   ...
   **마무리 안내 문구(D104):** 앱이 아래 문구를 넣는다. `[승인]`은 intake에서 `[의도 승인]`으로 바꾼다. verify에서는 Work 완료 화면(시나리오 7-3)에 실제로 있는 전달 버튼 이름으로 바꾼다. 전달 버튼이 `[완료만]`뿐이면 `[완료만]`이다.
...
- verify 승인 = Work 완료 화면의 전달 선택([완료만] / [push] / [PR 생성]). 시나리오 7로 이어진다.
- **형식 오류가 끝까지 남으면:** 승인 버튼을 비활성화하고 오류를 표시한다. 확인 창을 거치는 [오류 무시하고 승인]은 제공한다.
  ...
  - [승인]과 [오류 무시하고 승인]은 에이전트가 턴을 끝낸 뒤(승인 대기, 대기, 세션 종료)에 누를 수 있다. 무시한 오류의 기록과 handoff 머리글을 읽지 못했을 때의 처리는 D112를 따른다.
...
<RELAY_HOME>/                          # 기본 %USERPROFILE%\.relay, macOS/Linux ~/.relay
  ...
  projects/<project-id>/               # <레포 폴더 이름>-<레포 절대 경로 해시 6자>
    ...
    worktrees/<work-id>/
    works/<work-id>/
      work.json                        # Work 상태, 현재 단계, 기준 브랜치와 기준 커밋, 승인 기록(무시한 형식 오류 포함, D112), Work별 설정(자동 승인, 질문 방식), task별 형식 버전·프로세스 ID·시작 시각·Claude Code 버전(D105)·배포한 스킬 해시(D103), 진행 중 작업
...
- `project-id`는 레포 폴더 이름과 레포 절대 경로의 해시 앞 6자로 만든다(예: `my-api-3f9a1c`). 같은 이름의 레포가 여러 개여도 겹치지 않는다. 폴더 이름의 권한 규칙 패턴 문자(`[`, `]`, `*`, `?`, `\`)는 `_`로 바꾼다(D111).
...
- 결정이 없으면 항목 대신 "없음"을, [오류 무시하고 승인]에서 handoff 머리글을 읽지 못했으면 "handoff 머리글 오류로 결정을 읽지 못함"을 적는다(D112).
...
- `task_id`는 task 이벤트에만 있다. `payload`는 유형별 추가 정보이며 없으면 `{}`. [오류 무시하고 승인]이면 `task.approved`의 `payload`에 무시한 오류 수(`ignored_errors`)를 더한다(D112).
- 유형:
...
- **정적 검증:** `skills/check.mjs`가 머리글(D33), 크기(D95), 템플릿에 값을 채운 예시의 스키마 통과(D87), 설계와의 대조(산출물 템플릿의 절, 입력, 결정 지점, 사람이 정할 결정, 완료조건)를 확인한다. 모델은 부르지 않는다. 스킬이나 이 절을 고치면 함께 돌린다.
