...
- 짧은 스킬은 통과하면: S3 실패는 분량 초과 탓이다. D31의 한도(5,000토큰)를 지키면 된다.
...
- S3의 긴 시험용 스킬(채움 글 약 60줄)에서 끝부분을 잊은 원인은 분량으로 보인다. 다만 그 스킬이 5,000토큰을 실제로 넘었는지는 재지 않았다. 스킬을 구현할 때 합친 `SKILL.md`의 토큰 수를 재서 한도보다 충분히 작게 둔다.
